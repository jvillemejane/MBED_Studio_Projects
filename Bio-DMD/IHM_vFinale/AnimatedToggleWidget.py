...
class Toggle(QCheckBox):
    ...
    def __init__(self,
        ...
        unchecked_color="#ff0000"
        ):
        super().__init__(parent)

        # Save our properties on the object via self, so we can access them later
        # in the paintEvent.
        self._bar_brush = QBrush(QColor(unchecked_color).lighter())
...
class AnimatedToggle(Toggle):
    ...
    checked_circle_color = QPen(Qt.white)
    unchecked_circle_color = QPen(Qt.white)
    ...
    def paintEvent(self, e: QPaintEvent):
        ...
        p.setPen(self.checked_circle_color)
        barRect = QRectF(
            0, 0,
            contRect.width() - handleRadius, 0.40 * contRect.height()
        )
        ...
        if self.isChecked():
            ...
            p.setBrush(self._bar_brush)
            p.drawRoundedRect(barRect, rounding, rounding)
            p.setPen(self.unchecked_circle_color)
            p.setBrush(self._handle_brush)
